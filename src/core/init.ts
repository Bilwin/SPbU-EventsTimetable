...
export async function init(options: {
    ...
    mockTelegramEnv({
      onEvent(event, next) {
        if (event.name === 'web_app_request_theme') {
          let tp: ThemeParamsType | undefined;
          if (firstThemeSent) {
  ...
  const themeParamsMountAttempt = themeParams.mount.ifAvailable();
  if (themeParamsMountAttempt.ok) {
    themeParams.bindCssVars.ifAvailable();
  }

  miniApp.mount.ifAvailable();

  const viewportMountAttempt = viewport.mount.ifAvailable();
  if (viewportMountAttempt.ok) {
    try {
      await viewportMountAttempt.data;
